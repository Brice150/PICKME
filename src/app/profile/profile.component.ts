...
export class ProfileComponent implements OnInit, OnDestroy {
  ...
  ngOnDestroy() {
    ...
    this.getAllUserPicturesSubscription &&
      this.getAllUserPicturesSubscription.unsubscribe();
    ...
    this.deletePictureSubscription &&
      this.deletePictureSubscription.unsubscribe();
  }
  ...
  getPictures(fkUser: any) {
    this.pictures = [];
    this.getAllUserPicturesSubscription = this.pictureService
      .getAllUserPictures(fkUser)
      .subscribe({
        next: (response: Picture[]) => {
          if (response.length > 0) {
            for (let picture of response) {
              let reader = new FileReader();
              this.getPictureSubscription = this.pictureService
                .getPicture(picture.content.toString())
                .subscribe({
                  next: (event) => {
                    if (event.type === HttpEventType.Response) {
                      if (event.body instanceof Array) {
  ...
  addPicture(files: File[]) {
    ...
    this.addPictureSubscription = this.pictureService
      .addPicture(this.picturesData)
      .subscribe({
        next: (event) => {
          console.log(event);
        },
  ...
  pickMainPicture(picture: Picture) {
    ...
    this.toastr.info(
      'Please type password bottom page to update profile',
      'Profile',
      {
        positionClass: 'toast-bottom-center',
      }
  ...
  deletePicture(pictureId: number) {
    this.deletePictureSubscription = this.pictureService
